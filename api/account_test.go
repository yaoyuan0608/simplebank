...
func TestCreateAccouhntAPI(t *testing.T) {
	user, _ := randomUser(t)
	account := randomAccount(user.Username)
	...
	testCases := []struct {
	...
	}{
		{
			...
			checkResponse: func(rt *testing.T, recorder *httptest.ResponseRecorder) {
				// check response
				require.Equal(t, http.StatusOK, recorder.Code)
				requireBodyMatchAccount(t, recorder.Body, account)
			},
...
func TestListAccouhntAPI(t *testing.T) {
	user, _ := randomUser(t)
	...
	testCases := []struct {
		...
		},
		{
			...
			buildStubs: func(store *mockdb.MockStore) {
				...
				store.EXPECT().ListAccount(gomock.Any(), gomock.Eq(arg)).Times(0)
			},
			checkResponse: func(rt *testing.T, recorder *httptest.ResponseRecorder) {
				// check response
				require.Equal(t, http.StatusUnauthorized, recorder.Code)
		...
		},
		{
			...
			buildStubs: func(store *mockdb.MockStore) {
				store.EXPECT().ListAccount(gomock.Any(), gomock.Any()).Times(1).Return([]db.Account{}, sql.ErrConnDone)
			},
			checkResponse: func(rt *testing.T, recorder *httptest.ResponseRecorder) {
				// check response
				require.Equal(t, http.StatusInternalServerError, recorder.Code)
